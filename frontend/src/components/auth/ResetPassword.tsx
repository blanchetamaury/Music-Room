		...
		>
			<View style={styles.headerRow}>
				<Pressable onPress={() => onBack?.()} style={styles.backBtn} accessibilityRole="button">
					<ChevronLeft color={colorScheme === 'light' ? '#000000' : '#ffffff'}></ChevronLeft>
				</Pressable>
				<ThemedText
					type="title"
					style={[
						styles.title,
						{ color: `${colorScheme === 'light' ? '#000000' : '#ffffff'}`, marginLeft: 8 },
					]}
				>
					Reset password
				</ThemedText>
			...
			{!codeSent ? (
				<>
					...
					<Pressable
						...
						style={[
							styles.actionBtn,
							!email || isLoading ? styles.actionBtnDisabled : null,
							{
								maxWidth: Platform.OS === 'android' ? 280 : 'auto',
								minWidth: Platform.OS === 'android' ? 280 : 'auto',
							},
						]}
				...
				<Animated.View style={[styles.codeContainer, revealStyle]} pointerEvents={codeSent ? 'auto' : 'none'}>
					<ThemedText style={{ color: `${colorScheme === 'light' ? '#000000' : '#ffffff'}` }}>
						Enter the 6-digit code
					</ThemedText>
					<TextInput
						placeholder="------"
						placeholderTextColor={colorScheme === 'light' ? '#000000c0' : '#ffffffce'}
						keyboardType="number-pad"
						value={code}
						onChangeText={(v) => setCode(v.replace(/[^0-9]/g, '').slice(0, 6))}
						style={[styles.codeInput, { color: `${colorScheme === 'light' ? '#000000' : '#ffffff'}` }]}
						maxLength={6}
					/>
					...
					>
						...
						>
							...
							{showNewPassword == true && (
								<Eye color={colorScheme === 'light' ? '#000000' : '#ffffff'}></Eye>
							)}
						</Pressable>
					</InputPasswordForm>
