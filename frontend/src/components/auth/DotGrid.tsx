...
function createProgram(gl: ExpoWebGLRenderingContext, vertexSource: string, fragmentSource: string) {
      ...
      `;
			...
			for (let row = 0; row < rows; row++) {
				for (let column = 0; column < columns; column++) {
					...
					const phase = ((row * 37 + column * 73) % 100) / 100;

					vertices.push(x, y, phase);
				}
			}
			...
			const minSizeLocation = gl.getUniformLocation(program, 'u_minSize');

			const maxSizeLocation = gl.getUniformLocation(program, 'u_maxSize');

			const durationLocation = gl.getUniformLocation(program, 'u_pulseDuration');

			const intensityLocation = gl.getUniformLocation(program, 'u_pulseIntensity');
			...
			const color1 = hexToRgb(colors[0] ?? '#FFFFFF');

			const color2 = hexToRgb(colors[1] ?? colors[0] ?? '#FFFFFF');

			const color3 = hexToRgb(colors[2] ?? colors[1] ?? colors[0] ?? '#FFFFFF');
			...
			gl.uniform1f(minSizeLocation, minSize);

			gl.uniform1f(maxSizeLocation, maxSize);

			gl.uniform1f(durationLocation, pulseDuration);

			gl.uniform1f(intensityLocation, pulseIntensity);
		...
		[spacing, minSize, maxSize, colors, pulseDuration, pulseIntensity]
	);
