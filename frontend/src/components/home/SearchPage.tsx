...
export function SearchPage({ onNavigateHome }: SearchPageProps) {
	...
	return (
		...
		style={[homeStyles.searchInput, { color: '#fff' }, Platform.OS === 'web' ? ({ outlineWidth: 0, outlineColor: 'transparent', outlineStyle: 'none' } as any) : null,]}
		...
		{searchPlaylists.map((playlist) => (
			...
			<ThemedText style={homeStyles.playlistSongCount}>{playlist.songs} songs</ThemedText>
			</View>
			<Pressable
			onPress={(e) => {
				e.stopPropagation();
